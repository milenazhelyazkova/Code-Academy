...
printf("\n%dth/%d/%d, is the %dth day of the year", day, month, year, checkDay(year,month,day));
}
int checkDay(int year, int month, int day){
   if (month>2 && year % 4 == 0 && (year % 100 != 0 || year % 400 == 0)) /* if month is after february and the year is high */
         ++day;  /* add 1 day, for 29th of february */
   while (month-- > 0)
        day += days[month - 1]; 
   return day; 
} 
