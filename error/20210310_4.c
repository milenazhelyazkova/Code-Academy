...
някаква грешка записваме грешката във файла и името на функцията. след пускане на програмата провери файл task4.txt 
*/
#include <stdio.h>
#include <stdlib.h>
#include <errno.h>
...
int main()
{
    ...
    {
        double price = packPrice(20, 30);
        if (price != 0)
        {
            fprintf(fp, "%s %s\n", "double packPrice(int weight, int volume)", "success");
        }
        else
        {
            fprintf(fp, "%s %s\n", "double packPrice(int weight, int volume)", strerror(errno));
        }
    }
